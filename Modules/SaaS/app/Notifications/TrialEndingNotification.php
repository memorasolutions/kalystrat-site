...
class TrialEndingNotification extends TemplatedNotification
{
    ...
    protected function getFallbackMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ->subject(__("Fin de la période d'essai"))
            ->line(__("Votre période d'essai se termine le :date.", ['date' => $this->trialEndsAt]))
            ->line(__('Souscrivez à un abonnement pour continuer à profiter de toutes les fonctionnalités.'))
            ->action(__('Voir les plans'), url('/pricing'));
    }
}

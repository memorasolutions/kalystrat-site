                            ...
                            @foreach($widgets as $widget)
                                <li class="list-group-item d-flex justify-content-between align-items-center py-2 px-3" data-id="{{ $widget->id }}">
                                    ...
                                    <div class="d-flex gap-1">
                                        <a href="{{ route('admin.widgets.edit', $widget) }}" class="btn btn-sm btn-outline-warning p-1" title="{{ __('Modifier') }}" aria-label="{{ __('Modifier') }}">
                                            <i data-lucide="edit" style="width:14px;height:14px;"></i>
                                        </a>
                                        <form action="{{ route('admin.widgets.destroy', $widget) }}" method="POST" class="d-inline">
                                            @csrf @method('DELETE')
                                            <button type="button" class="btn btn-sm btn-outline-danger p-1" onclick="if(confirm('{{ __("Supprimer ce widget ?") }}')) this.form.submit()" title="{{ __('Supprimer') }}" aria-label="{{ __('Supprimer') }}">

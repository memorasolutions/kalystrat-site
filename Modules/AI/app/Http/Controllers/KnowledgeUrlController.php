...
class KnowledgeUrlController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('admin.ai.urls.index')
            ->with('success', __('Source URL ajoutée à la base de connaissances.'));
    }
    ...
    public function update(Request $request, KnowledgeUrl $url): RedirectResponse
    {
        ...
        return redirect()->route('admin.ai.urls.index')
            ->with('success', __('Source URL mise à jour.'));
    }
    ...
    public function destroy(KnowledgeUrl $url): RedirectResponse
    {
        ...
        return redirect()->route('admin.ai.urls.index')
            ->with('success', __('Source URL et tous ses documents supprimés.'));
    }
    ...
    public function checkRobots(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'allowed' => $allowed,
            'message' => $allowed
                ? __('Le scraping est autorisé par robots.txt.')
                : __('Le scraping est bloqué par robots.txt.'),
        ]);
    }
    ...
    public function scrape(KnowledgeUrl $url): RedirectResponse
    {
        ...
        if ($url->scrape_status === 'robots_blocked') {
            return redirect()->back()
                ->with('error', __('Scraping bloqué par robots.txt. Aucune page indexée.'));
        }

        if ($url->scrape_status === 'failed') {
            return redirect()->back()
                ->with('error', __('Le scraping a échoué : :error', ['error' => $url->scrape_error ?? __('erreur inconnue')]));
        }

        return redirect()->back()
            ->with('success', __(':count page(s) indexée(s) avec succès.', ['count' => $count]));
    }
}

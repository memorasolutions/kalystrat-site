...
class ChatBot extends Component
{
    ...
    public bool $showConsentBanner = false;
    ...
    public function mount(?string $page = null): void
    {
        ...
        // Visitor token for anonymous tracking (Loi 25 / GDPR compliant)
        if (! auth()->check()) {
            /** @var string $cookie */
            $cookie = request()->cookie('visitor_token') ?? '';
            $this->visitorToken = $cookie;
            $this->showConsentBanner = ! $this->visitorToken;
        }
    }

    public function acceptConsent(): void
    {
        $this->visitorToken = bin2hex(random_bytes(16));
        $this->showConsentBanner = false;
        cookie()->queue('visitor_token', $this->visitorToken, 60 * 24 * 30, '/', null, true, true, false, 'Lax');
    }
    ...
    {
        // Rate limiting: 1 message per 2 seconds per user/IP
        $throttleKey = 'chat:'.(auth()->id() ?: request()->ip());
        if (RateLimiter::tooManyAttempts($throttleKey, 1)) {
            $this->error = __('Veuillez patienter avant d\'envoyer un autre message.');
        ...
        $this->isLoading = true;
        $this->dispatch('fetch-bot-response', userMessage: $userMessage);
    }

    #[On('fetch-bot-response')]
    public function fetchBotResponse(string $userMessage): void
    {
        try {
    ...
    private function finalizeLeadCapture(): void
    {
        if (class_exists(ContactMessage::class)) {
            ...
            Notification::route('mail', config('mail.from.address'))
                ->notify(new ChatLeadNotification($contactMessage));
        } else {
    ...
    private function loadConversation(): void
    {
        if (auth()->check()) {
            $conversation = AiConversation::where('user_id', auth()->id())
                ...
                ->latest()
                ->first();
        } elseif ($this->visitorToken) {
            $conversation = AiConversation::where('visitor_token', $this->visitorToken)
                ->active()
                ->latest()
                ->first();
        } else {
    ...
    {
        if (auth()->check() || $this->visitorToken) {
            if (! $this->conversationId) {

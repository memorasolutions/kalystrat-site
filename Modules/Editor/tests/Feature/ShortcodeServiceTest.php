...
it('render_shortcodes global helper delegates to ShortcodeService', function () {
    Shortcode::factory()->create([
        'tag' => 'helper_test',
        'html_template' => '<b>{{ $content }}</b>',
        'is_active' => true,
    ...
    $result = render_shortcodes('[helper_test]via helper[/helper_test]');

    expect($result)->toBe('<b>via helper</b>');
});

it('loadShortcodes lazy loads only once — DB queried a single time across two renders', function () {
    Shortcode::factory()->create([
        'tag' => 'lazy',
        'html_template' => '<i>{{ $content }}</i>',
        'is_active' => true,
    ]);
    ...
    DB::listen(function ($query) use (&$queryCount) {
        if (str_contains($query->sql, 'shortcodes')) {
            $queryCount++;
        }
    });

    $service->render('[lazy]first[/lazy]');
    $service->render('[lazy]second[/lazy]');

    // loadShortcodes() should only hit the DB once (lazy-loaded on first render)

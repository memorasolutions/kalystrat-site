...
@extends('backoffice::themes.backend.layouts.admin', ['title' => __('Modifier l\'article'), 'subtitle' => __('Blog')])
                ...
                <div class="card-body p-4">
                    ...
                    @if($article->status === 'published')
                        <form action="{{ route('admin.blog.articles.unpublish', $article->id) }}" method="POST" class="mt-2">
                            @csrf
                            <button type="submit" class="w-100 btn btn-outline-warning">{{ __('Dépublier') }}</button>
                        </form>
                    @else
                        <form action="{{ route('admin.blog.articles.publish', $article->id) }}" method="POST" class="mt-2">
                            @csrf
                            <button type="submit" class="w-100 btn btn-outline-success">{{ __('Publier') }}</button>
                        </form>
                    @endif
                </div>
            </div>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="mb-0">
                        ...
                        <input type="text" class="form-control @error('content_password') is-invalid @enderror" id="content_password" name="content_password" value="{{ old('content_password', $article->content_password ?? '') }}" placeholder="{{ __('Laisser vide pour accès libre') }}">
            ...
            <div class="card">
                ...
                <div class="card-body p-4">
                    ...
                    <input type="file" name="featured_image"
                           class="form-control"
                           accept="image/*">
                    <div class="form-text text-muted">{{ __('Laissez vide pour conserver l\'image actuelle') }}</div>
                </div>
            </div>
        </div>
    </div>

...
<form action="{{ route('admin.blog.articles.update', $article) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row gy-3">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        <x-editor::tiptap name="content" :value="old('content', $article->content ?? '')" :label="__('Contenu')" />
                    </div>
                    <div class="mb-0">
                        <label class="form-label">{{ __('Extrait') }}</label>
        ...
        <div class="col-md-4">
            ...
            <div class="card mb-3">
                <div class="card-header">
                    <h6 class="mb-0">{{ __('Publication') }}</h6>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <div class="btn-group w-100" role="group" aria-label="{{ __('Statut de publication') }}">
                            ...
                            <label class="btn btn-outline-secondary" for="status-draft">{{ __('Brouillon') }}</label>
                            ...
                            <label class="btn btn-outline-success" for="status-published">{{ __('Publié') }}</label>
                            ...
                            <label class="btn btn-outline-warning" for="status-archived">{{ __('Archivé') }}</label>
                        </div>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="d-flex gap-3 mt-4">
                        ...
                        <a href="{{ route('admin.blog.articles.index') }}" class="btn btn-outline-secondary">{{ __('Annuler') }}</a>
                    </div>
                    @if($article->status === 'published')
                        <form action="{{ route('admin.blog.articles.unpublish', $article->id) }}" method="POST" class="mt-2">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-outline-warning w-100">{{ __('Dépublier') }}</button>
                        </form>
                    @else
                        <form action="{{ route('admin.blog.articles.publish', $article->id) }}" method="POST" class="mt-2">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-outline-success w-100">{{ __('Publier') }}</button>
                        </form>
                    @endif
                </div>
            </div>
            ...
            <div class="card mb-3">
                <div class="card-header">
                    <h6 class="mb-0">{{ __('Catégorie et tags') }}</h6>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label for="category-select" class="form-label">{{ __('Catégorie') }}</label>
                        <select id="category-select" name="category_id" class="form-select" aria-label="{{ __('Sélectionner une catégorie') }}">
                            <option value="">{{ __('— Sélectionner —') }}</option>
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('category_id', $article->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                        <div class="form-text">{{ __('Tapez pour rechercher ou créer une catégorie') }}</div>
                    </div>
                    <div class="mb-0">
                        <label for="tags-select" class="form-label">{{ __('Tags') }}</label>
                        ...
                        <div class="form-text">{{ __('Tapez pour rechercher ou créer un tag') }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h6 class="mb-0">{{ __('Image mise en avant') }}</h6>
                </div>
                <div class="card-body">
                    ...
                    <div class="form-text">{{ __('Laissez vide pour conserver l\'image actuelle') }}</div>
                </div>
            </div>
        </div>
    </div>

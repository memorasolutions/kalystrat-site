...
<div>
    @if($articles->isEmpty())
        <div class="text-center py-20 text-gray-400">
            <p class="text-lg">{{ __('Aucun article pour le moment.') }}</p>
        </div>
    @else
        ...
        <div class="mt-10 text-center">
            <button wire:click="loadMore"
                ...
                <span wire:loading.remove>{{ __('Charger plus d\'articles') }}</span>
                <span wire:loading>{{ __('Chargement...') }}</span>
            </button>
        </div>
        @else
        <div class="mt-10 text-center text-sm text-gray-400">
            {{ __('Tous les articles sont affichés.') }}
        </div>
        @endif
    @endif
</div>

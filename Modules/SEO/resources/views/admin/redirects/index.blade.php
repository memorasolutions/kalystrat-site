...
@section('title', __('Redirections URL'))
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.redirects.index') }}" class="mb-4">
                <div class="input-group">
                    ...
                    <input type="text" name="search" class="form-control"
                           placeholder="{{ __('Rechercher par URL source ou destination...') }}"
                           value="{{ request('search') }}">
                    <button class="btn btn-outline-secondary" type="submit">{{ __('Rechercher') }}</button>
                    @if(request('search'))
                        <a href="{{ route('admin.redirects.index') }}" class="btn btn-outline-danger">{{ __('Effacer') }}</a>
                    @endif
                </div>
            </form>
            ...
            @if($redirects->isEmpty())
                <div class="text-center py-5">
                    ...
                    <h5 class="text-muted">{{ __('Aucune redirection configurée.') }}</h5>
                    <p class="text-muted">{{ __('Commencez par ajouter votre première redirection.') }}</p>
                </div>
            @else
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        <thead>
                            <tr>
                                <th>{{ __('URL source') }}</th>
                                <th>{{ __('URL destination') }}</th>
                                <th>{{ __('Code') }}</th>
                                <th>{{ __('Hits') }}</th>
                                <th>{{ __('Dernier hit') }}</th>
                                <th>{{ __('Statut') }}</th>
                                    ...
                                    <td class="text-end">
                                        <div class="btn-group btn-group-sm">
                                            ...
                                            <form action="{{ route('admin.redirects.destroy', $redirect) }}"
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-outline-danger btn-sm" aria-label="{{ __('Supprimer') }}">
            ...
            <div class="modal-content">
                <form id="redirectForm" method="POST" action="{{ route('admin.redirects.store') }}">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="redirectModalLabel">{{ __('Ajouter une redirection') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="{{ __('Fermer') }}"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="from_url" class="form-label">{{ __('URL source') }} <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="from_url" name="from_url"
                                   placeholder="/ancien-url" required>
                            <div class="form-text">{{ __('Utilisez * pour les wildcards (ex: /ancien/*)') }}</div>
                        </div>

                        <div class="mb-3">
                            <label for="to_url" class="form-label">{{ __('URL destination') }} <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="to_url" name="to_url"
                                   placeholder="/nouveau-url" required>
                        </div>

                        <div class="mb-3">
                            <label for="status_code" class="form-label">{{ __('Code de redirection') }}</label>
                            <select class="form-select" id="status_code" name="status_code">
                                <option value="301">{{ __('301 - Permanent') }}</option>
                                <option value="302">{{ __('302 - Temporaire') }}</option>
                                <option value="307">{{ __('307 - Temporaire (préserve méthode)') }}</option>
                                <option value="308">{{ __('308 - Permanent (préserve méthode)') }}</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="is_active">{{ __('Redirection active') }}</label>
                            </div>
                        </div>

                        <div class="mb-0">
                            <label for="note" class="form-label">{{ __('Note (optionnel)') }}</label>
                            <textarea class="form-control" id="note" name="note" rows="2"
                                      placeholder="{{ __('Raison de la redirection...') }}"></textarea>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.edit-redirect').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            title.textContent = @json(__('Modifier la redirection'));
    ...
    modal.addEventListener('show.bs.modal', function(e) {
        if (!e.relatedTarget || !e.relatedTarget.classList.contains('edit-redirect')) {
            title.textContent = @json(__('Ajouter une redirection'));

        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-3">
                    <h6 class="mb-0">{{ __('Nouveau workflow') }}</h6>
                ...
                <div class="card-body">
                    <div class="row mb-3">
                        <div class="col-md-8">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="{{ __('Ex: Série de bienvenue') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" rows="2" class="form-control @error('description') is-invalid @enderror" placeholder="{{ __('Description du workflow...') }}">{{ old('description') }}</textarea>
            ...
            <div class="card mt-3">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h6 class="mb-0">{{ __('Étapes du workflow') }}</h6>
                ...
                <div class="card-body">
                    <div id="stepsContainer">
                        <div class="text-center text-muted py-4" id="emptySteps">
                            ...
                            <p>{{ __('Aucune étape. Cliquez sur « Ajouter une étape » pour commencer.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-3 d-flex gap-3">
                ...
                <a href="{{ route('admin.newsletter.workflows.index') }}" class="btn btn-outline-secondary">{{ __('Annuler') }}</a>
            </div>
        </div>

        <div class="col-lg-4">
            <div class="card">
                <div class="card-header"><h6 class="mb-0">{{ __('Informations') }}</h6></div>
                <div class="card-body">
                    <p class="text-muted text-sm mb-2">Le workflow sera créé en statut <strong>{{ __('brouillon') }}</strong>.</p>
                    <p class="text-muted text-sm mb-0">{{ __('Activez-le depuis la page de détails pour commencer l\'inscription automatique des abonnés.') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    div.innerHTML = `
        <div class="card-body py-2 px-3">
            <div class="d-flex align-items-center gap-2 mb-2">
                ...
                <button type="button" class="btn btn-sm btn-outline-danger ms-auto" onclick="this.closest('.step-item').remove()" aria-label="{{ __('Supprimer') }}">
                    <i data-lucide="x"></i>
                </button>
            </div>
            <div class="step-config-${stepIndex}">
                ...
                ${type === 'action' ? `<input type="text" name="steps[${stepIndex}][config][action_type]" class="form-control form-control-sm" placeholder="{{ __('Type d\'action') }}" value="${config.action_type || ''}">` : ''}
            </div>
        </div>
    `;

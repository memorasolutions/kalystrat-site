...
@extends('admintabler::layouts.admin', ['title' => __('Workflows'), 'subtitle' => __('Marketing')])
            ...
            <div class="card-body">
                @if($workflows->isEmpty())
                    <div class="text-center py-5">
                        ...
                        <p class="text-muted">{{ __('Aucun workflow pour l\'instant.') }}</p>
                        <a href="{{ route('admin.newsletter.workflows.create') }}" class="btn btn-sm btn-outline-primary">{{ __('Créer un workflow') }}</a>
                    </div>
                @else
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">{{ __('Inscrits') }}</th>
                                    <th>{{ __('Statut') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex justify-content-end gap-1">
                                            <a href="{{ route('admin.newsletter.workflows.show', $workflow) }}" class="btn btn-sm btn-outline-info" title="{{ __('Voir') }}" aria-label="{{ __('Voir') }}">
                                                <i data-lucide="bar-chart-3"></i>
                                            </a>
                                            <a href="{{ route('admin.newsletter.workflows.edit', $workflow) }}" class="btn btn-sm btn-outline-primary" title="{{ __('Modifier') }}" aria-label="{{ __('Modifier') }}">
                                                <i data-lucide="pencil"></i>
                                            </a>
                                            <form method="POST" action="{{ route('admin.newsletter.workflows.destroy', $workflow) }}" onsubmit="return confirm('{{ __("Supprimer ce workflow ?") }}')">
                                                @csrf @method('DELETE')

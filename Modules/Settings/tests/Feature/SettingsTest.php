...
test('settings seeder creates default settings', function () {
    $this->seed(SettingsDatabaseSeeder::class);
...
test('Settings::set and get roundtrip via facade', function () {
    Settings::set('roundtrip_key', 'roundtrip_value');

    expect(Settings::get('roundtrip_key'))->toBe('roundtrip_value');
});

test('Settings::forget removes the key from DB', function () {
    Settings::set('forget_key', 'to_be_removed');
    expect(Settings::has('forget_key'))->toBeTrue();

    $service = app(SettingsService::class);
    $service->forget('forget_key');

    expect(Setting::where('key', 'forget_key')->exists())->toBeFalse();
});

test('Settings cache is invalidated after set so fresh value is returned', function () {
    // Prime the cache with an initial value
    Settings::set('cache_test_key', 'initial');
    expect(Settings::get('cache_test_key'))->toBe('initial');

    // Update the value — Setting::set() calls Cache::forget internally
    Settings::set('cache_test_key', 'updated');

    // The cache should now return the new value, not the stale cached one
    expect(Settings::get('cache_test_key'))->toBe('updated');
});

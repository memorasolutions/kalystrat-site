...
<div>
    ...
    <div class="d-block d-md-flex align-items-center justify-content-between mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            <button type="button" class="btn btn-sm btn-primary d-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#addKeyModal" title="{{ __('Ajouter une nouvelle clé') }}">
        ...
        <div class="card-body py-3 px-4">
            <div class="row g-3 align-items-center">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control form-control-sm" id="translationSearch" wire:model.live.debounce.300ms="search" placeholder="{{ __('Rechercher...') }}" aria-label="{{ __('Rechercher une clé ou traduction') }}">
                </div>
                <div class="col-md-3">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header border-bottom py-3 px-4">
                    <h6 class="modal-title" id="addKeyModalLabel">{{ __('Ajouter une clé de traduction') }}</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
                </div>
                <div class="modal-body py-3 px-4">
                    <div class="mb-3">
                        <label for="newKey" class="form-label">Clé</label>
                        <input type="text" class="form-control" id="newKey" wire:model="newKey" placeholder="{{ __('ex: Welcome message') }}">
                    </div>
                    <div class="mb-3">
                        <label for="newSourceValue" class="form-label">Valeur FR</label>
                        <input type="text" class="form-control" id="newSourceValue" wire:model="newSourceValue" placeholder="{{ __('ex: Bienvenue') }}">
                    </div>
                    <div class="mb-0">
                        <label for="newTargetValue" class="form-label">Traduction ({{ strtoupper($targetLocale) }})</label>
                        <input type="text" class="form-control" id="newTargetValue" wire:model="newTargetValue" placeholder="{{ __('ex: Welcome') }}">
                    </div>
                </div>
                <div class="modal-footer border-top py-3 px-4">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">{{ __('Annuler') }}</button>
                    <button type="button" class="btn btn-primary" wire:click="addKey" data-bs-dismiss="modal">{{ __('Ajouter') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="addLocaleModal" tabindex="-1" aria-labelledby="addLocaleModalLabel" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header border-bottom py-3 px-4">
                    <h6 class="modal-title" id="addLocaleModalLabel">{{ __('Ajouter une langue') }}</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
                </div>
                <div class="modal-body py-3 px-4">
                    <div class="mb-0">
                        <label for="newLocaleInput" class="form-label">Code locale (ISO 639-1)</label>
                        <input type="text" class="form-control" id="newLocaleInput" wire:model="newLocale" placeholder="{{ __('ex: es, de, pt') }}" maxlength="2">
                        <small class="text-muted">Code à 2 lettres uniquement.</small>
                    </div>
                </div>
                <div class="modal-footer border-top py-3 px-4">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">{{ __('Annuler') }}</button>
                    <button type="button" class="btn btn-primary" wire:click="addLocale" data-bs-dismiss="modal">{{ __('Ajouter') }}</button>
                </div>
            </div>
        </div>
    </div>

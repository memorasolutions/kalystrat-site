        ...
        <table class="w-full text-left text-sm">
            <thead class="border-b border-gray-200 text-xs uppercase text-gray-500 dark:border-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th class="px-6 py-3">{{ __('Rôles') }}</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($users as $user)
                    <tr class="text-gray-900 dark:text-white">
                        ...
                        <td class="px-6 py-4">
                            @if($user->is_active)
                                <span class="rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-800 dark:bg-green-900 dark:text-green-300">{{ __('Actif') }}</span>
                            @else
                                <span class="rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900 dark:text-red-300">{{ __('Inactif') }}</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="flex gap-2 px-6 py-4">
                            <a href="{{ route('admin.users.show', $user) }}" class="text-gray-600 hover:underline dark:text-gray-400">{{ __('Voir') }}</a>
                            <a href="{{ route('admin.users.edit', $user) }}" class="text-indigo-600 hover:underline dark:text-indigo-400">{{ __('Modifier') }}</a>
                            @if($user->id !== auth()->id())
                                <form method="POST" action="{{ route('admin.users.destroy', $user) }}" onsubmit="return confirm('{{ __("Supprimer cet utilisateur ?") }}')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:underline dark:text-red-400">{{ __('Supprimer') }}</button>
                                </form>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">{{ __('Aucun utilisateur trouvé.') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

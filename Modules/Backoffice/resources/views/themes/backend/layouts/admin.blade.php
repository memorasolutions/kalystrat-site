...
<head>
    ...
    {{-- Splash screen --}}
    <link href="{{ asset('splash-screen-nobleui.css') }}" rel="stylesheet">
...
<body class="sidebar-dark" data-base-url="{{ url('/') }}">
    ...
    @livewire('backoffice-command-palette')
        ...
        <div class="page-wrapper">
            ...
            <main id="main-content" class="page-content container-xxl">
                ...
                @hasSection('breadcrumbs')
                    @yield('breadcrumbs')
                @else
                    @inject('breadcrumbService', 'Modules\Backoffice\Services\BreadcrumbService')
                    @php $autoBreadcrumbs = $breadcrumbService->generate(); @endphp
                    @if(! empty($autoBreadcrumbs))
                        <x-backoffice::breadcrumb :items="$autoBreadcrumbs" />
                    @endif
                @endif
    ...
    <div x-data="{ cmdkOpen: false }"
         @keydown.window.meta.k.prevent="cmdkOpen = true; $nextTick(() => { const i = document.getElementById('cmdk-input'); if (i) i.focus(); })"
         @keydown.window.ctrl.k.prevent="cmdkOpen = true; $nextTick(() => { const i = document.getElementById('cmdk-input'); if (i) i.focus(); })"
         @keydown.escape.window="cmdkOpen = false"
         x-cloak>
        <div x-show="cmdkOpen" x-transition.opacity class="position-fixed top-0 start-0 w-100 h-100" style="z-index:9999;background:rgba(0,0,0,.5);" @click.self="cmdkOpen = false">
            <div class="position-absolute start-50 translate-middle-x" style="top:20%;width:100%;max-width:520px;padding:0 16px;">
                <div class="bg-body rounded-3 shadow-lg border overflow-hidden">
                    <div class="p-3 border-bottom d-flex align-items-center gap-2">
                        <i data-lucide="search" style="width:18px;height:18px;" class="text-muted flex-shrink-0" aria-hidden="true"></i>
                        <input id="cmdk-input" type="text" class="form-control border-0 shadow-none p-0 fs-5" placeholder="{{ __('Rechercher...') }} (⌘K)" aria-label="{{ __('Recherche rapide') }}"
                               @input="$dispatch('cmdk-search', { query: $event.target.value })">
                        <kbd class="bg-body-secondary rounded px-2 py-1 small text-muted flex-shrink-0">ESC</kbd>
                    </div>
                    <div class="p-2" style="max-height:400px;overflow-y:auto;">
                        @livewire('backoffice-global-search', key('cmdk-search'))
                    </div>
                </div>
            </div>
        </div>

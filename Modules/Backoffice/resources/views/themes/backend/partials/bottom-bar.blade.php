...
<nav class="admin-bottom-bar d-lg-none" id="bottomBar" aria-label="{{ __('Navigation mobile') }}">
    <div class="d-flex h-100 align-items-stretch">
        ...
        <a href="{{ route('admin.users.index') }}" class="bottom-bar-item flex-fill d-flex flex-column justify-content-center align-items-center text-decoration-none position-relative {{ request()->routeIs('admin.users.*', 'admin.teams.*') ? 'active' : '' }}" aria-label="{{ __('Équipe') }}" {{ request()->routeIs('admin.users.*', 'admin.teams.*') ? 'aria-current=page' : '' }}>
            <span class="position-relative">
                <i data-lucide="users" class="mb-1" aria-hidden="true"></i>
                @if($unreadNotifs > 0)
                <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="font-size:9px;min-width:16px;padding:2px 4px;">{{ $unreadNotifs > 99 ? '99+' : $unreadNotifs }}<span class="visually-hidden">{{ __('notifications non lues') }}</span></span>
                @endif
            </span>
        ...
        <a href="{{ route('admin.notifications.index') }}" class="bottom-bar-item flex-fill d-flex flex-column justify-content-center align-items-center text-decoration-none position-relative {{ request()->routeIs('admin.notifications.*') ? 'active' : '' }}" aria-label="{{ __('Notifications') }} {{ $unreadCount > 0 ? '('.$unreadCount.' non lues)' : '' }}" {{ request()->routeIs('admin.notifications.*') ? 'aria-current=page' : '' }}>
            <span class="position-relative">
                <i data-lucide="bell" class="mb-1" aria-hidden="true"></i>
                @if($unreadCount > 0)
                <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="font-size:9px;min-width:16px;padding:2px 4px;">{{ $unreadCount > 99 ? '99+' : $unreadCount }}<span class="visually-hidden">{{ __('notifications non lues') }}</span></span>
                @endif
            </span>

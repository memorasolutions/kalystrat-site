...
<div>
    ...
    <div class="border-bottom pb-3 mb-3">
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <select wire:model.live="filterCauser" class="form-select form-select-sm w-auto" aria-label="{{ __('Filtrer par utilisateur') }}">
                <option value="">{{ __('Tous les utilisateurs') }}</option>
                @foreach($users as $u)
                    <option value="{{ $u->id }}">{{ $u->name }}</option>
                @endforeach
            </select>
            <select wire:model.live="filterLogName" class="form-select form-select-sm w-auto" aria-label="{{ __('Filtrer par journal') }}">
                <option value="">{{ __('Tous les journaux') }}</option>
                @foreach($logNames as $logName)
                    <option value="{{ $logName }}">{{ $logName }}</option>
                @endforeach
            </select>
            <select wire:model.live="filterEvent" class="form-select form-select-sm w-auto" aria-label="{{ __('Filtrer par événement') }}">

...
@php
$helpTextsData = [
    'ai.openrouter_api_key'  => __('Votre clé API OpenRouter. Créez un compte gratuit sur openrouter.ai, puis copiez votre clé depuis le tableau de bord. Les modèles gratuits ne coûtent rien.'),
    'ai.default_model'       => __("Le modèle IA utilisé par défaut quand aucun modèle spécifique n'est défini pour une tâche. Choisissez un modèle gratuit pour commencer."),
    'ai.chatbot_model'       => __('Le modèle utilisé pour le chatbot intégré au site. Un modèle conversationnel comme Llama est recommandé.'),
    'ai.content_model'       => __("Le modèle utilisé pour générer du contenu (articles, résumés). Un modèle orienté code/contenu fonctionne bien."),
    'ai.moderation_model'    => __('Le modèle qui analyse les commentaires et contenus soumis pour détecter le spam ou les messages toxiques.'),
    'ai.seo_model'           => __('Le modèle qui génère les balises meta SEO (titre, description) pour vos articles et pages.'),
    'ai.translation_model'   => __('Le modèle utilisé pour la traduction automatique des clés de traduction. Llama 3.3 gère bien le français, anglais, espagnol, allemand.'),
    'ai.temperature'         => __('Contrôle la créativité des réponses IA (0 = précis et déterministe, 2 = très créatif). Pour la traduction, gardez 0.3-0.5. Pour le contenu, 0.7 est un bon équilibre.'),
    'ai.max_tokens'          => __("Nombre maximum de mots/tokens que l'IA peut générer par réponse. 2048 convient pour la plupart des usages. Augmentez pour les articles longs."),
    'ai.chatbot_enabled'     => __('Active ou désactive le chatbot IA visible sur votre site public.'),
    'ai.chatbot_system_prompt' => __("Les instructions données au chatbot pour définir son comportement. Exemple : Tu es un assistant pour une boutique de vélos."),
];
@endphp
<div x-data="{
                                ...
                                <div class="col-md-7">
                                    ...
                                    @elseif(str_contains($setting->key, 'color'))
                                        <div class="d-flex gap-2 align-items-center">
                                            <input type="color"
                                                   ...
                                                   aria-label="{{ __('Couleur') }} {{ ucwords(str_replace(['branding.', '_', '.'], ['', ' ', ' '], $setting->key)) }}">
                                            <input type="text"
                                                   wire:model="values.{{ $setting->id }}"
                                                   class="form-control"
                                                   style="width:130px;"
                                                   placeholder="#000000"
                                                   aria-label="{{ __('Valeur hexadécimale') }} {{ ucwords(str_replace(['branding.', '_', '.'], ['', ' ', ' '], $setting->key)) }}">
                                        </div>
                                    ...
                                    @elseif(str_contains($setting->key, 'mail_from_address'))
                                        <input type="email"
                                               ...
                                               aria-label="{{ __('Adresse email expéditeur') }}">
                                    ...
                                    @elseif(str_contains($setting->key, 'mail_port'))
                                        <input type="number"
                                               ...
                                               aria-label="{{ __('Port serveur mail') }}">
                                    ...
                                    @elseif($setting->key === 'mail_encryption')
                                        <select wire:model="values.{{ $setting->id }}"
                                                ...
                                                aria-label="{{ __('Chiffrement mail') }}">
                                    ...
                                    @elseif(str_contains($setting->key, '_model') && str_starts_with($setting->key, 'ai.'))
                                        <select wire:model="values.{{ $setting->id }}"
                                                ...
                                                aria-label="{{ __('Modèle IA') }} {{ ucwords(str_replace(['ai.', '_'], ['', ' '], $setting->key)) }}">
                                            @php
                                                $aiModels = [

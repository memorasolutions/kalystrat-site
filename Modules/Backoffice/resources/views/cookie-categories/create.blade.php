    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.cookie-categories.store') }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="{{ __('ex: preferences') }}">
                @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="mb-3">
                <label for="label" class="form-label">Label affiché</label>
                <input type="text" name="label" id="label" class="form-control @error('label') is-invalid @enderror" value="{{ old('label') }}" required placeholder="{{ __('ex: Cookies de préférences') }}">

...
class CookieCategoryController
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('admin.cookie-categories.index')->with('success', __('Catégorie créée avec succès.'));
    }
    ...
    public function update(Request $request, CookieCategory $cookieCategory): RedirectResponse
    {
        ...
        return redirect()->route('admin.cookie-categories.index')->with('success', __('Catégorie mise à jour avec succès.'));
    }

    public function destroy(CookieCategory $cookieCategory): RedirectResponse
    {
        if ($cookieCategory->isRequired()) {
            return redirect()->route('admin.cookie-categories.index')->with('error', __('Impossible de supprimer une catégorie obligatoire.'));
        }

        $cookieCategory->delete();

        return redirect()->route('admin.cookie-categories.index')->with('success', __('Catégorie supprimée avec succès.'));
    }
}

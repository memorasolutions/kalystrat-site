...
class PluginController
{
    ...
    public function toggle(string $name): RedirectResponse
    {
        ...
        if (in_array($name, self::PROTECTED_MODULES, true)) {
            return back()->with('error', __('Le module :name est protégé et ne peut pas être désactivé.', ['name' => $name]));
        }

        if (! Module::has($name)) {
            return back()->with('error', __("Le module :name n'existe pas.", ['name' => $name]));
        }

        if (Module::isEnabled($name)) {
            $dependents = $this->getEnabledDependents($name);
            if ($dependents !== []) {
                return back()->with('error', __('Impossible de désactiver :name : les modules suivants en dépendent : :dependents.', ['name' => $name, 'dependents' => implode(', ', $dependents)]));
            }
            Module::disable($name);

            return back()->with('success', __('Module :name désactivé.', ['name' => $name]));
        }

        $missing = $this->getMissingDependencies($name);
        if ($missing !== []) {
            return back()->with('error', __("Impossible d'activer :name : les dépendances suivantes sont inactives : :missing.", ['name' => $name, 'missing' => implode(', ', $missing)]));
        }
        Module::enable($name);

        return back()->with('success', __('Module :name activé.', ['name' => $name]));
    }

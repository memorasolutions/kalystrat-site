...
class ActivityLogsTable extends Component
{
    ...
    public function render(): View
    {
        ...
        $users = cache()->remember('activity_log_users', 3600, fn () => User::orderBy('name')->get(['id', 'name']));
        $logNames = cache()->remember('activity_log_names', 3600, fn () => Activity::distinct()->orderBy('log_name')->pluck('log_name'));
        $events = cache()->remember('activity_log_events', 3600, fn () => Activity::distinct()->whereNotNull('event')->orderBy('event')->pluck('event'));

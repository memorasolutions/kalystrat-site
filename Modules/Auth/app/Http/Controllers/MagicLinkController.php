...
class MagicLinkController extends Controller
{
    ...
    public function sendLink(Request $request): RedirectResponse
    {
        ...
        $rateLimitKey = 'magic-link-email:'.hash('xxh128', $request->email);
    ...
    public function sendSms(Request $request): RedirectResponse
    {
        ...
        $smsRateLimitKey = 'magic-link-sms:'.hash('xxh128', $request->email);
        if (RateLimiter::tooManyAttempts($smsRateLimitKey, 1)) {
            $seconds = RateLimiter::availableIn($smsRateLimitKey);

            return back()->withErrors(['sms' => __('SMS déjà envoyé. Réessayez dans :seconds secondes.', ['seconds' => $seconds])]);
        }

        $user = User::where('email', $request->email)->first();
        if (! $user || ! $user->phone) {
            return back()->withErrors(['sms' => __('Aucun numéro de téléphone associé à ce compte.')]);
        }
        ...
        $expiryMinutes = (int) Settings::get('magic_link_expiry_minutes', 15);
        $message = __('Votre code de connexion : :token. Valide :minutes minutes.', ['token' => $token, 'minutes' => $expiryMinutes]);
        $sent = $smsDriver->send($user->phone, $message);

        if (! $sent) {
            return back()->withErrors(['sms' => __('Échec de l\'envoi du SMS. Veuillez réessayer.')]);
        }

        RateLimiter::hit($smsRateLimitKey, 600);

        return back()->with('sms_sent', __('Code envoyé par SMS.'));
    }
    ...
    public function verify(Request $request): RedirectResponse
    {
        ...
        if (! $user) {
            return back()->withErrors(['token' => __('Code invalide ou expiré.')])->withInput();
        }

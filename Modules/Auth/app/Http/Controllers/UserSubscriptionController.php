...
class UserSubscriptionController extends Controller
{
    ...
    public function cancel(SubscriptionService $service): RedirectResponse
    {
        ...
        if ($endsAt) {
            return back()->with('success', __('Abonnement annulé. Il restera actif jusqu\'au :date.', ['date' => $endsAt]));
        }

        return back()->with('error', __('Aucun abonnement actif à annuler.'));
    }
    ...
    public function resume(SubscriptionService $service): RedirectResponse
    {
        ...
        if ($service->resume($user)) {
            return back()->with('success', __('Abonnement réactivé.'));
        }

        return back()->with('error', __('L\'abonnement ne peut pas être réactivé.'));
    }

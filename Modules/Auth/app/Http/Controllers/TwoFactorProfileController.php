...
class TwoFactorProfileController extends Controller
{
    ...
    public function setup(): View|RedirectResponse
    {
        ...
        if ($user->hasEnabledTwoFactor()) {
            return redirect()->route('user.profile')->with('info', __('Double authentification déjà activée.'));
        }
    ...
    public function confirm(Request $request): RedirectResponse
    {
        ...
        if ($this->twoFactor->confirm(auth()->user(), $request->code)) {
            return redirect()->route('user.two-factor.recovery-codes')
                ->with('success', __('Double authentification activée !'));
        }

        return back()->withErrors(['code' => __('Code invalide. Vérifiez votre application authenticator.')]);
    }
    ...
    public function disable(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('user.profile')->with('success', __('Double authentification désactivée.'));
    }
    ...
    public function regenerateRecoveryCodes(): RedirectResponse
    {
        ...
        return redirect()->route('user.two-factor.recovery-codes')
            ->with('success', __('Codes de secours régénérés.'));
    }
}

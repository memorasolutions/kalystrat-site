...
<p class="mt-2 text-base text-gray-700">{{ __('Entrez votre courriel pour recevoir un code à 6 caractères.') }}</p>
...
@if(session('status'))
    <div class="bg-green-50 border border-green-200 text-green-800 p-3 rounded-md text-sm mt-4 flex flex-col gap-1" role="alert">
        ...
        <a href="{{ route('magic-link.verify') }}?email={{ urlencode(old('email', '')) }}"
           class="font-semibold text-sky-800 transition-all duration-200 hover:text-sky-800 hover:underline">
...
<form action="{{ route('magic-link.send') }}" method="POST" class="mt-8">
    ...
    <div class="mt-6 text-center">
        <a href="{{ route('login') }}" class="font-medium text-sky-800 transition-all duration-200 hover:text-sky-800 focus:text-sky-800 hover:underline">

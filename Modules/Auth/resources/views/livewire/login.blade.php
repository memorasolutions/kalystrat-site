...
<div>
    ...
    @if(Settings::get('branding.login_subtitle'))
        <p class="mt-2 text-base text-gray-700">{{ Settings::get('branding.login_subtitle') }}</p>
    @else
        <p class="mt-2 text-base text-gray-700">{{ __('Accédez à votre compte') }}</p>
    @endif
        ...
        <div class="space-y-5">
            ...
            <div>
                ...
                <div class="mt-2 relative">
                    <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                        <i class="ti ti-mail text-gray-400 text-xl" aria-hidden="true"></i>
                    </div>
                    <input wire:model="email" type="email" id="login-email" autocomplete="email"
                           ...
                           placeholder="{{ __('votre@courriel.com') }}" required autofocus aria-required="true" aria-describedby="login-email-error">
                </div>
                @error('email')<p class="text-red-600 text-sm mt-1" id="login-email-error" role="alert">{{ $message }}</p>@enderror
            </div>
            ...
            <div>
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('password.request') }}" class="text-sm font-medium text-sky-800 transition-all duration-200 hover:text-sky-800 focus:text-sky-800 hover:underline inline-flex items-center" style="min-height:24px" wire:navigate>
                        {{ __('Mot de passe oublié ?') }}
                    </a>
                </div>
                <div class="mt-2 relative">
                    <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                        <i class="ti ti-lock text-gray-400 text-xl" aria-hidden="true"></i>
                    </div>
                    <input wire:model="password" type="password" id="login-password" autocomplete="current-password"
                           ...
                           placeholder="{{ __('Mot de passe') }}" required aria-required="true" aria-describedby="login-password-error">
                    <button type="button"
                        ...
                        <i class="ti ti-eye text-xl toggle-eye-login-password" aria-hidden="true"></i>
                    </button>
                </div>
                @error('password')<p class="text-red-600 text-sm mt-1" id="login-password-error" role="alert">{{ $message }}</p>@enderror
            </div>

            {{-- Remember me --}}
            <div class="flex items-center" style="gap: 0.75rem">
                <input wire:model="remember" type="checkbox" id="remember"
                       class="text-sky-800 border-gray-300 rounded focus:ring-sky-500" style="width: 1.5rem; height: 1.5rem; flex-shrink: 0">
    ...
    <div x-data="{ supported: typeof window.browserSupportsWebAuthn === 'function' && window.browserSupportsWebAuthn() }" x-show="supported" x-cloak x-bind:inert="!supported" @if($isLocked) style="display:none" @endif>
        <div class="relative my-6">
            ...
            <div class="relative flex justify-center text-sm"><span class="px-4 bg-white text-gray-600">{{ __('ou') }}</span></div>
        </div>
        <x-authenticate-passkey redirect="/admin">
            <button type="button"
                ...
                <i class="ti ti-fingerprint text-xl" aria-hidden="true"></i>
    ...
    <div class="mt-6 text-center space-y-2">
        <p class="text-sm text-gray-700">
            {{ __('Pas de compte ?') }}
            <a href="{{ route('register') }}" class="font-medium text-sky-800 transition-all duration-200 hover:text-sky-800 focus:text-sky-800 hover:underline" wire:navigate>{{ __('Créer un compte') }}</a>
        </p>
        <p class="text-sm text-gray-700">
            <a href="{{ route('magic-link.request') }}" class="font-medium text-sky-800 transition-all duration-200 hover:text-sky-800 focus:text-sky-800 hover:underline" wire:navigate>{{ __('Connexion sans mot de passe (code)') }}</a>
        </p>
    </div>
</div>

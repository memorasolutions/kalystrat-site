                ...
                <tbody>
                    @forelse($submissions as $submission)
                        <tr class="{{ $submission->isNew() ? 'fw-bold' : '' }}">
                            ...
                            <td>
                                @if($submission->isNew())
                                    <span class="badge bg-primary">{{ __('Nouveau') }}</span>
                                @else
                            ...
                            <td class="text-end">
                                <div class="d-inline-flex gap-1">
                                    ...
                                    <form action="{{ route('admin.formbuilder.forms.submissions.destroy', [$form, $submission]) }}" method="POST" class="d-inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-sm btn-danger" onclick="if(confirm('{{ __('Supprimer ?') }}')) this.form.submit()">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-4 text-muted">{{ __('Aucune soumission.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

...
@section('title', __('Modifier') . ' : ' . $form->title)

@section('content')
<div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
    <h4 class="mb-3 mb-md-0">{{ __('Modifier') }} : {{ $form->title }}</h4>
</div>
...
<form action="{{ route('admin.formbuilder.forms.update', $form) }}" method="POST">
    ...
    <div class="row">
        <div class="col-md-4 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">{{ __('Paramètres') }}</h6>
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <a href="{{ route('admin.formbuilder.forms.index') }}" class="btn btn-outline-secondary">{{ __('Annuler') }}</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-8 grid-margin stretch-card" x-data="{
            ...
            removeField(i) { if(confirm('{{ __("Supprimer ce champ ?") }}')) this.fields.splice(i, 1); }
        }">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h6 class="card-title mb-0">{{ __('Champs du formulaire') }}</h6>
                        <button type="button" class="btn btn-sm btn-success" @click="addField()">
                            <i data-lucide="plus" style="width: 16px; height: 16px;"></i> {{ __('Ajouter') }}
                        </button>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th>{{ __('Nom') }}</th>
                                    <th>{{ __('Type') }}</th>
                                    <th style="width:70px">{{ __('Requis') }}</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select class="form-select form-select-sm" :name="'fields['+i+'][type]'" x-model="field.type">
                                                ...
                                                <option value="select">{{ __('Liste') }}</option>
                                                <option value="checkbox">{{ __('Case à cocher') }}</option>
                                                <option value="radio">{{ __('Boutons radio') }}</option>
                        ...
                        <p x-show="fields.length === 0" class="text-center text-muted py-3">{{ __('Aucun champ. Cliquez sur « Ajouter ».') }}</p>
                    </div>
                </div>
            </div>
        </div>

                            ...
                            <td class="text-end">
                                <div class="d-inline-flex gap-1">
                                    ...
                                    <form action="{{ route('admin.formbuilder.forms.destroy', $form) }}" method="POST" class="d-inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-sm btn-danger" onclick="if(confirm('{{ __('Supprimer ce formulaire et toutes ses soumissions ?') }}')) this.form.submit()">
                        ...
                        <tr>
                            <td colspan="5">
                                <div class="text-center py-5 text-muted">
                                    ...
                                    <h5 class="mb-1">{{ __('Aucun formulaire') }}</h5>
                                    <p class="mb-3">{{ __('Créez votre premier formulaire pour collecter des données.') }}</p>
                                    <a href="{{ route('admin.formbuilder.forms.create') }}" class="btn btn-primary btn-sm">
                                        <i data-lucide="plus" style="width:14px;height:14px;" class="me-1"></i> {{ __('Créer un formulaire') }}
                                    </a>
                                </div>
                            </td>
                        </tr>

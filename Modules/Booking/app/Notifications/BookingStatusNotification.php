...
class BookingStatusNotification extends TemplatedNotification
{
    ...
    {
        $subject = __(self::SUBJECTS[$this->newStatus] ?? 'Mise à jour de votre rendez-vous');
        $message = __(self::MESSAGES[$this->newStatus] ?? 'Le statut de votre rendez-vous a été modifié.');

        return (new MailMessage)
            ->subject($subject)
            ->line($message)
            ->line(__('**Service :** :service', ['service' => $this->appointment->service->name]))
            ->line(__('**Date :** :date', ['date' => $this->appointment->start_at->locale('fr')->isoFormat('dddd D MMMM YYYY a HH:mm')]))
            ->action(__('Gérer mon rendez-vous'), route('booking.manage', $this->appointment->cancel_token));
    }
}

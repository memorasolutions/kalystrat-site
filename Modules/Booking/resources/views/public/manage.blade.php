...
<head>
    ...
    <title>{{ __('Gerer mon rendez-vous') }}</title>
    ...
    <div class="container" style="max-width: 600px; margin-top: 3rem;">
        ...
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-body p-4">
                <h4 class="mb-4">{{ __('Mon rendez-vous') }}</h4>
                ...
                <hr>
                <h6 class="text-danger mb-3">{{ __('Annuler ce rendez-vous') }}</h6>
                <form action="{{ route('booking.cancel', $appointment->cancel_token) }}" method="POST" onsubmit="return confirm('{{ __("Êtes-vous sûr de vouloir annuler ?") }}')">
                    @csrf
                    <div class="mb-3">
                        <textarea name="reason" class="form-control" rows="2" placeholder="{{ __('Raison de l\'annulation (optionnel)') }}"></textarea>
                    </div>
                    <button type="submit" class="btn btn-danger">{{ __('Annuler le rendez-vous') }}</button>
                </form>
                @endif
            </div>
        </div>

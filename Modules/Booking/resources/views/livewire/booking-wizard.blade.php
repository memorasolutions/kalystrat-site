...
<div>
    ...
    @if($step === 2)
    <div>
        <button wire:click="goBack" class="btn btn-sm btn-outline-secondary mb-3">{{ __('← Retour') }}</button>
        <h6 class="mb-3">{{ __('Choisissez une date') }}</h6>
        <div class="row g-2">
    ...
    <div>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-7">
                <h6 class="mb-3">{{ __('Vos coordonnées') }}</h6>
                <div class="row g-3">
                    <div class="col-6">
                        <input type="text" wire:model="firstName" class="form-control @error('firstName') is-invalid @enderror" placeholder="{{ __('Prénom *') }}">
                        @error('firstName') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-6">
                        <input type="text" wire:model="lastName" class="form-control @error('lastName') is-invalid @enderror" placeholder="{{ __('Nom *') }}">
                        @error('lastName') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-12">
                        <input type="email" wire:model="email" class="form-control @error('email') is-invalid @enderror" placeholder="{{ __('Courriel *') }}">
                        @error('email') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-12">
                        <input type="tel" wire:model="phone" class="form-control @error('phone') is-invalid @enderror" placeholder="{{ __('Téléphone') }}">
                        @error('phone') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-12">
                        <textarea wire:model="notes" class="form-control" rows="2" placeholder="{{ __('Notes (optionnel)') }}"></textarea>
                    </div>
                    <div class="col-12">
                        <button wire:click="submitBooking" wire:loading.attr="disabled" class="btn btn-primary w-100" {{ !$selectedTime ? 'disabled' : '' }}>
                            <span wire:loading.remove>{{ __('Confirmer le rendez-vous') }}</span>
    ...
    <div class="text-center py-4">
        ...
        <h4 class="fw-bold mb-2">{{ __('Rendez-vous confirmé!') }}</h4>

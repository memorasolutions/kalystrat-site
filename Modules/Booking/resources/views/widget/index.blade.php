                ...
                <div class="card shadow-sm">
                    <div class="card-body p-4">
                        ...
                        <div id="step1" class="step active">
                            <h6 class="mb-3">{{ __('Choisissez un service') }}</h6>
                        ...
                        <div id="step2" class="step">
                            <h6 class="mb-3">{{ __('Choisissez une date et une heure') }}</h6>
                            <div class="mb-3">
                                <label class="form-label">Date</label>
                                <select class="form-select" id="dateSelect" onchange="updateTimeSlots()">
                                    <option value="">{{ __('Selectionner une date') }}</option>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="timeSelect" disabled>
                                    <option value="">{{ __('Selectionner une heure') }}</option>
                                </select>
                            </div>
                        ...
                        <div id="step3" class="step">
                            <h6 class="mb-3">{{ __('Vos informations') }}</h6>
                            <form id="bookingForm">
                                <div class="row g-3">
                                    <div class="col-6">
                                        <input type="text" class="form-control" name="first_name" placeholder="{{ __('Prénom') }}" required>
                                    </div>
                                    <div class="col-6">
                                    ...
                                    <div class="col-12">
                                        <input type="tel" class="form-control" name="phone" placeholder="{{ __('Téléphone') }}">
                                    </div>
                                    <div class="col-12">
                                        <textarea class="form-control" name="notes" rows="2" placeholder="{{ __('Notes (facultatif)') }}"></textarea>
                                    </div>
                                </div>
                        ...
                        <div id="success" class="text-center py-4" style="display: none;">
                            ...
                            <h5 class="mt-3">{{ __('Rendez-vous confirmé') }}</h5>
...
function nextStep(n) {
    if (n === 2 && !selectedService) return alert('{{ __("Veuillez choisir un service.") }}');
    if (n === 3 && (!document.getElementById('dateSelect').value || !document.getElementById('timeSelect').value))
        return alert('{{ __("Veuillez choisir une date et une heure.") }}');
    goStep(n);
}
...
function updateTimeSlots() {
    ...
    ts.innerHTML = '<option value="">{{ __('Selectionner une heure') }}</option>';
    ts.disabled = !date;
    if (date && slots[date]) {
...
document.getElementById('bookingForm').addEventListener('submit', function(e) {
    ...
    fetch('/api/booking', {
        ...
        if (r.ok) {
            ...
            resize();
        } else { alert('{{ __("Erreur. Veuillez réessayer.") }}'); }
    }).catch(function() { alert('{{ __("Erreur réseau.") }}'); });
});

            ...
            <div class="card">
                ...
                <form method="POST"
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                ...
                                @foreach(['text' => __('Texte court'), 'textarea' => __('Zone de texte'), 'select' => __('Liste deroulante'), 'checkbox' => __('Case a cocher'), 'radio' => __('Boutons radio')] as $val => $lbl)
                        ...
                        <div class="mb-3" id="options-field" style="display: none;">
                            ...
                            <textarea class="form-control @error('options') is-invalid @enderror" id="options" name="options" rows="4" placeholder="{{ __('Option 1&#10;Option 2') }}">{{ old('options', isset($intakeQuestion) && is_array($intakeQuestion->options) ? implode("\n", $intakeQuestion->options) : '') }}</textarea>
                            @error('options')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="card-footer">
                        ...
                        <a href="{{ route('admin.booking.intake-questions.index', $service) }}" class="btn btn-secondary">{{ __('Annuler') }}</a>
                    </div>
                </form>
            </div>
        </div>

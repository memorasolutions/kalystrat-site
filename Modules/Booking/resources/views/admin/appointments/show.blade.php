...
@extends('backoffice::themes.backend.layouts.admin', ['title' => __('Détail du rendez-vous'), 'subtitle' => __('Réservations')])
    ...
    <div class="col-lg-8">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <h6>{{ __('Thérapeute assigné') }}</h6>
                        <p>{{ $appointment->assignedAdmin?->name ?? 'Non assigné' }}</p>
                    </div>
                    <div class="col-md-6">
                        <h6>{{ __('Assigner un thérapeute') }}</h6>
                ...
                <div class="mb-3 d-flex gap-2">
                    <form action="{{ route('admin.booking.appointments.approve', $appointment) }}" method="POST" class="d-inline">
                        @csrf @method('PUT')
                        <input type="text" name="note" class="form-control form-control-sm d-inline-block mb-2" placeholder="{{ __('Note (optionnel)') }}" style="max-width:300px;">
                        <button type="submit" class="btn btn-sm btn-success" aria-label="{{ __('Approuver') }}"><i data-lucide="check"></i> {{ __('Approuver') }}</button>
                    </form>
                    <form action="{{ route('admin.booking.appointments.reject', $appointment) }}" method="POST" class="d-inline">
                        @csrf @method('PUT')
                        <input type="text" name="reason" class="form-control form-control-sm d-inline-block mb-2" placeholder="{{ __('Raison du rejet') }}" required style="max-width:300px;">
                        <button type="submit" class="btn btn-sm btn-danger" aria-label="{{ __('Rejeter') }}"><i data-lucide="x"></i> {{ __('Rejeter') }}</button>
                    </form>
                </div>
                @endif

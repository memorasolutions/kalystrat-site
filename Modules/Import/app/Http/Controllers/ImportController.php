...
class ImportController extends Controller
{
    ...
    public function preview(Request $request): View
    {
        ...
        $fullPath = Storage::disk('local')->path('imports/'.$fileName);
    ...
    public function execute(Request $request): RedirectResponse
    {
        ...
        $basePath = Storage::disk('local')->path('imports');

        if (! str_starts_with((string) realpath($fullPath), (string) realpath($basePath))) {
            return redirect()->route('admin.import.index')
                ->with('error', __('Chemin de fichier invalide.'));
        }

        if (! file_exists($fullPath)) {
            return redirect()->route('admin.import.index')
                ->with('error', __('Le fichier uploadé n\'existe plus. Veuillez réessayer.'));
        }
        ...
        $message = __(':imported/:total éléments importés avec succès.', ['imported' => $result->imported, 'total' => $result->total]);

        if ($result->skipped > 0) {
            $message .= ' '.__(':skipped lignes ignorées.', ['skipped' => $result->skipped]);
        }

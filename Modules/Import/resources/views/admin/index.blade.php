...
@section('title', __('Import de données'))
@section('content')
<div class="page-content">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <h4 class="mb-0">{{ __('Import de données') }}</h4>
    </div>
        ...
        <div class="col-md-4">
            <div class="card text-center h-100">
                <div class="card-body">
                    ...
                    <h6>{{ __('Choisir le type') }}</h6>
                    <p class="text-muted small mb-0">{{ __('Sélectionnez le type de données à importer') }}</p>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card text-center h-100">
                <div class="card-body">
                    ...
                    <h6>{{ __('Téléverser le fichier') }}</h6>
                    <p class="text-muted small mb-0">{{ __('Téléchargez votre fichier CSV ou Excel') }}</p>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card text-center h-100">
                <div class="card-body">
                    ...
                    <h6>{{ __('Mapper les colonnes') }}</h6>
                    <p class="text-muted small mb-0">{{ __('Associez les colonnes aux champs du modèle') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.import.preview') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label for="model_type" class="form-label">{{ __('Type de données') }} *</label>
                ...
                <div class="mb-3">
                    <label for="file" class="form-label">{{ __('Fichier à importer') }} *</label>
                    <input type="file" class="form-control @error('file') is-invalid @enderror" id="file" name="file" accept=".csv,.xlsx,.xls" required>
                    @error('file')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <div class="form-text">{{ __('Formats acceptés : CSV, XLSX. Taille maximale : 10 Mo.') }}</div>
                </div>

                <div class="mb-4">
                    <label class="form-label">{{ __('Télécharger un modèle :') }}</label>
                ...
                <button type="submit" class="btn btn-primary">
                    <i data-lucide="eye" style="width: 16px; height: 16px;"></i> {{ __('Prévisualiser') }}
                </button>
            </form>
        </div>
    </div>

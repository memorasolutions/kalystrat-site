...
@extends('admintabler::layouts.admin', ['title' => __('Témoignages'), 'subtitle' => __('Contenu')])
            ...
            <div id="testimonial-list" class="px-2 pb-3">
                ...
                <div class="testimonial-item d-flex align-items-center gap-3 p-3 mx-2 mb-2 border rounded" data-id="{{ $testimonial->id }}">
                    ...
                    <div class="d-flex gap-1">
                        <a href="{{ route('admin.testimonials.edit', $testimonial) }}" class="btn btn-sm btn-outline-primary" title="{{ __('Modifier') }}" aria-label="{{ __('Modifier') }}">
                            <i data-lucide="pencil"></i>
                        </a>
                        <form action="{{ route('admin.testimonials.destroy', $testimonial) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-outline-danger" title="{{ __('Supprimer') }}" aria-label="{{ __('Supprimer') }}" onclick="return confirm('{{ __('Supprimer ce témoignage ?') }}')">

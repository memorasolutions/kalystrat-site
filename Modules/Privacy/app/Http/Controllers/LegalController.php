...
class LegalController extends Controller
{
    ...
    public function rightsRequestStore(Request $request): RedirectResponse
    {
        ...
        if ($dpoEmail) {
            try {
                Mail::raw(
                    __("Nouvelle demande d'exercice de droits")."\n\n"
                    .__('Reference :')." {$rightsRequest->reference}\n"
                    .__('Type :')." {$rightsRequest->request_type}\n"
                    .__('De :')." {$rightsRequest->name} ({$rightsRequest->email})\n"
                    .__('Date limite :')." {$rightsRequest->deadline_at->format('Y-m-d')}\n\n"

...
class SystemAlertNotification extends TemplatedNotification
{
    ...
    protected function getFallbackMail(object $notifiable): MailMessage
    {
        $greeting = match ($this->level) {
            'info' => __('Information système'),
            'warning' => __('Avertissement système'),
            'critical' => __('Alerte critique'),
            default => __('Alerte système'),
        };

        return (new MailMessage)
            ->subject(__('Alerte système - :app', ['app' => config('app.name')]))
            ->greeting($greeting)
            ->line($this->message)

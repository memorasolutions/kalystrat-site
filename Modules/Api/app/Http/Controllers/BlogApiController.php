...
final class BlogApiController extends BaseApiController
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $this->applyFilters($query, $request, ['category', 'user_id']);
        $this->applySorting($query, $request, ['created_at', 'title', 'views_count'], '-created_at');

        $articles = $query->paginate($this->getPerPage($request));
    ...
    {
        $categories = Cache::remember('api.blog.categories', config('cache-ttl.api_blog_categories', 3600), fn () => Article::published()

...
class MetricAggregatorService
{
    ...
    /** @return array<string, list<MetricWidget>> */
    public function getGroupedWidgets(): array
    {
        $groups = [];
        foreach ($this->getProviders() as $provider) {
            try {
                $widgets = $provider->getWidgets();
                $hasData = collect($widgets)->contains(fn (MetricWidget $w) => $w->value !== '0' && $w->value !== '0.00 CAD' && $w->value !== '0.0%');
                if ($hasData || empty($groups)) {
                    $groups[$provider->getMetricName()] = $widgets;
                }
            } catch (\Throwable $e) {
                Log::warning("MetricProvider {$provider->getMetricName()} failed: {$e->getMessage()}");
            }

...
class SearchService
{
    ...
    public function searchAdmin(string $query, string $type = 'all', int $limit = 10): array
    {
        ...
        if (($type === 'all' || $type === 'articles') && class_exists(Article::class)) {
        ...
        if (($type === 'all' || $type === 'pages') && class_exists(StaticPage::class)) {
            $results['pages'] = StaticPage::search($query)->take($limit)->get();
        }

        if (($type === 'all' || $type === 'plans') && class_exists(Plan::class)) {
        ...
        if (($type === 'all' || $type === 'categories') && class_exists(Category::class)) {
    ...
    public function searchNavbar(string $query, int $limit = 3): array
    {
        ...
        if (class_exists(Article::class)) {
            $results['articles'] = Article::search($query)->take($limit)->get();
        } else {
    ...
    public function searchFront(string $query, int $perPage = 10): array
    {
        ...
        $pages = class_exists(StaticPage::class)
            ? StaticPage::search($query)
                ->query(fn ($q) => $q->where('status', 'published'))
                ->paginate($perPage)
            : new LengthAwarePaginator([], 0, $perPage);

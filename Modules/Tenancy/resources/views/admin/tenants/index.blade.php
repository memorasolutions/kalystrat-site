                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <div class="d-flex gap-1 justify-content-end">
                                    <a href="{{ route('admin.tenants.show', $tenant) }}" class="btn btn-sm btn-outline-secondary" title="{{ __('Voir') }}" aria-label="{{ __('Voir') }}">
                                        <i data-lucide="eye"></i>
                                    </a>
                                    <a href="{{ route('admin.tenants.edit', $tenant) }}" class="btn btn-sm btn-outline-primary" title="{{ __('Modifier') }}" aria-label="{{ __('Modifier') }}">
                                        <i data-lucide="edit"></i>
                                    </a>
                                    <form action="{{ route('admin.tenants.destroy', $tenant) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-sm btn-outline-danger" title="{{ __('Supprimer') }}" aria-label="{{ __('Supprimer') }}" onclick="if(confirm('{{ __('Supprimer ce tenant ?') }}')) this.closest('form').submit()">

...
it('start and complete lifecycle transitions status correctly', function () {
    $experiment = Experiment::factory()->create(['status' => 'draft']);
    ...
    $experiment->complete('control');
    $fresh = $experiment->fresh();
    expect($fresh->status)->toBe('completed')
        ->and($fresh->winner)->toBe('control')
        ->and($fresh->ended_at)->not->toBeNull();
});

it('complete with winner not in variants still saves winner', function () {
    $experiment = Experiment::factory()->running()->create([
        'variants' => ['control', 'variant_a'],
    ]);

    // complete() accepts any string winner — no validation in the model
    $experiment->complete('nonexistent_variant');

    expect($experiment->fresh()->status)->toBe('completed')
        ->and($experiment->fresh()->winner)->toBe('nonexistent_variant');
});

it('slug is unique — duplicate slug causes a database exception', function () {
    Experiment::factory()->create(['slug' => 'my-unique-slug']);

    expect(fn () => Experiment::factory()->create(['slug' => 'my-unique-slug']))
        ->toThrow(QueryException::class);
});

it('variants array must contain at least two items to be meaningful', function () {
    $experiment = Experiment::factory()->create([
        'variants' => ['control', 'variant_a', 'variant_b'],
    ]);

    expect($experiment->variants)->toBeArray()
        ->and(count($experiment->variants))->toBeGreaterThanOrEqual(2);
});

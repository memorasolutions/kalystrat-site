...
<form action="{{ route('admin.short-urls.store') }}" method="POST">
        ...
        <div class="col-xl-8">
            <div class="card">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="mb-3">
                        <label for="slug" class="form-label fw-semibold">{{ __('Slug personnalisé') }}</label>
                        <div class="input-group">
                            <span class="input-group-text">/s/</span>
                            <input type="text" name="slug" id="slug"
                                   class="form-control @error('slug') is-invalid @enderror"
                                   value="{{ old('slug') }}"
                                   placeholder="{{ __('Laisser vide pour auto-générer') }}">
                            @error('slug')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-text text-muted">{{ __('Lettres, chiffres, tirets et underscores uniquement.') }}</div>
                    </div>
                    <div>
                        ...
                        <input type="text" name="title" id="title"
                               ...
                               value="{{ old('title') }}"
                               placeholder="{{ __('Titre descriptif (usage interne)') }}">
                        @error('title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="redirect_type" id="redirect_type" class="form-select">
                            <option value="302" @selected(old('redirect_type', '302') === '302')>302 - {{ __('Temporaire') }}</option>
                            <option value="301" @selected(old('redirect_type') === '301')>301 - {{ __('Permanente') }}</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="max_clicks" id="max_clicks"
                               ...
                               value="{{ old('max_clicks') }}" min="1" placeholder="{{ __('Illimité') }}">
                        @error('max_clicks')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="password" class="form-label fw-semibold">{{ __('Protection par mot de passe') }}</label>
                        <input type="password" name="password" id="password"
                               class="form-control @error('password') is-invalid @enderror"
                               placeholder="{{ __('Laisser vide = pas de protection') }}">
                        @error('password')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="tags" id="tags" class="form-control"
                               value="{{ old('tags') }}" placeholder="{{ __('tag1, tag2, tag3') }}">
    ...
    <div class="d-flex align-items-center gap-3 mt-4">
        <button type="submit" class="btn btn-primary">{{ __('Créer le lien') }}</button>

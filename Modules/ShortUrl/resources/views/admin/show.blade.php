...
@extends('backoffice::themes.backend.layouts.admin', ['title' => __('Statistiques'), 'subtitle' => '/s/' . $shortUrl->slug])
    ...
    <div class="card-body p-4 d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div class="d-flex align-items-center gap-3 flex-wrap">
            <div>
                ...
                <button type="button" class="btn btn-sm btn-outline-primary ms-2" aria-label="{{ __('Copier le lien') }}"
                        onclick="navigator.clipboard.writeText('{{ url('/s/' . $shortUrl->slug) }}');this.textContent='{{ __('Copié !') }}';setTimeout(()=>this.textContent='{{ __('Copier') }}',1500);">
                    {{ __('Copier') }}
                </button>
            </div>
            <span class="badge {{ $shortUrl->is_active ? 'bg-success' : 'bg-secondary' }}">
                {{ $shortUrl->is_active ? __('Actif') : __('Inactif') }}
            </span>
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('admin.short-urls.edit', $shortUrl) }}" class="btn btn-primary btn-sm">
                <i data-lucide="edit" style="width:14px;height:14px;" class="me-1"></i>{{ __('Modifier') }}
            </a>
            <a href="{{ route('admin.short-urls.index') }}" class="btn btn-outline-secondary btn-sm">{{ __('Retour') }}</a>
        </div>
    </div>
</div>
...
<div class="row g-4 mb-4">
    @php
        ...
        $stats = [
            ['icon' => 'mouse-pointer', 'label' => __('Total clics'), 'value' => number_format($analytics['total_clicks']), 'color' => 'primary'],
            ['icon' => 'smartphone', 'label' => __('Appareils'), 'value' => count($analytics['devices']), 'color' => 'info'],
            ['icon' => 'globe', 'label' => __('Pays'), 'value' => count($analytics['countries']), 'color' => 'success'],
            ['icon' => 'clock', 'label' => __('Dernier clic'), 'value' => $lastClick ? $lastClick->clicked_at->diffForHumans() : __('Aucun'), 'color' => 'warning'],
        ];
    @endphp
...
<div class="row g-4 mb-4">
    ...
    <div class="col-xl-8">
        <div class="card h-100">
            <div class="card-header py-3 px-4 border-bottom">
                <h5 class="card-title mb-0 fw-semibold">{{ __('Clics par jour (30 derniers jours)') }}</h5>
            </div>
            <div class="card-body p-4">
                ...
                @else
                    <div class="text-center py-4">
                        ...
                        <p class="text-muted mb-0">{{ __('Aucun clic sur les 30 derniers jours.') }}</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="col-xl-4">
        <div class="card mb-4">
            <div class="card-header py-3 px-4 border-bottom">
                <h5 class="card-title mb-0 fw-semibold">{{ __('Appareils') }}</h5>
            </div>
            <div class="card-body p-4">
                @forelse($analytics['devices'] as $device)
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <span class="text-capitalize">{{ $device['device_type'] ?? __('Inconnu') }}</span>
                ...
                @empty
                    <p class="text-muted small mb-0">{{ __('Aucune donnée.') }}</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header py-3 px-4 border-bottom">
                <h5 class="card-title mb-0 fw-semibold">{{ __('Navigateurs') }}</h5>
            </div>
            <div class="card-body p-4">
                @forelse($analytics['browsers'] as $browser)
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <span>{{ $browser['browser'] ?? __('Inconnu') }}</span>
...
<div class="row g-4">
    <div class="col-xl-6">
        <div class="card">
            <div class="card-header py-3 px-4 border-bottom">
                <h5 class="card-title mb-0 fw-semibold">{{ __('Sources de trafic') }}</h5>
            </div>
            <div class="card-body p-0">
                @if(count($analytics['top_referrers']) > 0)
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            <thead>
                                <tr><th class="ps-4">{{ __('Referrer') }}</th><th class="text-end pe-4">{{ __('Clics') }}</th></tr>
                            </thead>
                            <tbody>
                ...
                @else
                    <div class="text-center py-4">
                        <p class="text-muted mb-0">{{ __('Aucun referrer enregistré.') }}</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="col-xl-6">
        <div class="card">
            <div class="card-header py-3 px-4 border-bottom">
                <h5 class="card-title mb-0 fw-semibold">{{ __('Pays') }}</h5>
            </div>
            <div class="card-body p-0">
                @if(count($analytics['countries']) > 0)
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            <thead>
                                <tr><th class="ps-4">{{ __('Code pays') }}</th><th class="text-end pe-4">{{ __('Clics') }}</th></tr>
                            </thead>
                            <tbody>
                                @foreach($analytics['countries'] as $country)
                                    <tr>
                                        <td class="ps-4">{{ $country['country_code'] ?? __('Inconnu') }}</td>
                ...
                @else
                    <div class="text-center py-4">
                        <p class="text-muted mb-0">{{ __('Aucune donnée géographique.') }}</p>
                    </div>
                @endif
            </div>
        </div>

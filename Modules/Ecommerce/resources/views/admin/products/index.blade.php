...
@extends('admintabler::layouts.admin', ['title' => __('Produits'), 'subtitle' => __('Boutique')])
...
<div class="card">
    <div class="card-header py-3 px-4 border-bottom d-flex justify-content-between align-items-center">
        <form action="{{ route('admin.ecommerce.products.index') }}" method="GET" class="d-flex gap-2">
            ...
            <button type="submit" class="btn btn-sm btn-outline-primary" aria-label="{{ __('Rechercher') }}"><i data-lucide="search" class="icon-sm"></i></button>
        </form>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end pe-4">
                            <a href="{{ route('admin.ecommerce.products.edit', $product) }}" class="btn btn-sm btn-light" aria-label="{{ __('Modifier') }}"><i data-lucide="edit-2" class="icon-sm"></i></a>
                            <form action="{{ route('admin.ecommerce.products.destroy', $product) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('Supprimer ce produit ?') }}')">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-light text-danger" aria-label="{{ __('Supprimer') }}"><i data-lucide="trash-2" class="icon-sm"></i></button>
                            </form>
                        </td>
                    </tr>
                    @empty

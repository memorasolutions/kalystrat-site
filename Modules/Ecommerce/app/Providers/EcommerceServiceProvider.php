...
use Modules\Ecommerce\Listeners\SendOrderDeliveredNotification;
use Modules\Ecommerce\Listeners\SendOrderRefundNotification;
use Modules\Ecommerce\Listeners\SendOrderShippedNotification;
use Modules\Ecommerce\Services\Tax\CanadaTaxCalculator;
use Modules\Ecommerce\Services\Tax\EuVatCalculator;
use Modules\Ecommerce\Services\Tax\UsTaxCalculator;
...
class EcommerceServiceProvider extends BaseModuleServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        Event::listen(OrderRefunded::class, SendOrderRefundNotification::class);
        Event::listen(OrderRefunded::class, DispatchEcommerceWebhooks::class);
        Event::listen(OrderDelivered::class, SendOrderDeliveredNotification::class);
        Event::listen(OrderDelivered::class, DispatchEcommerceWebhooks::class);

        Blade::anonymousComponentPath(
            module_path('Ecommerce', 'resources/views/components'),
            'ecommerce'
        );
        ...
        if (config('modules.ecommerce.abandoned_cart.enabled', false)) {
            $this->app->booted(function () {
                ...
                $schedule->job(new ProcessAbandonedCarts)->hourly()->withoutOverlapping();
            });
        }
    }
    ...
    public function register(): void
    {
        ...
        $calculator = match (config('modules.ecommerce.tax_jurisdiction', 'ca')) {
            'us' => UsTaxCalculator::class,
            'eu' => EuVatCalculator::class,
            default => CanadaTaxCalculator::class,
        };
        $this->app->bind(TaxCalculatorInterface::class, $calculator);

...
class CheckoutApiController extends BaseApiController
{
    ...
    public function embeddedCheckout(Request $request): JsonResponse
    {
        $validated = $request->validate([
            'shipping_address_id' => ['required', 'integer', 'exists:ecommerce_addresses,id'],
            'billing_address_id' => ['nullable', 'integer', 'exists:ecommerce_addresses,id'],
        ...
        $session = $this->checkoutService->createEmbeddedCheckoutSession($order, $validated['return_url']);
    ...
    public function sessionStatus(Request $request): JsonResponse
    {
        ...
        $status = $this->checkoutService->getCheckoutSessionStatus($validated['session_id']);

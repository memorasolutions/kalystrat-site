...
class OrderConfirmationNotification extends TemplatedNotification
{
    ...
    protected function getFallbackMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ->subject(__('Confirmation de commande #:number', ['number' => $this->order->order_number]))
            ->greeting(__('Bonjour :name,', ['name' => $notifiable->name]))
            ->line(__('Nous avons bien reçu votre commande #:number.', ['number' => $this->order->order_number]))
            ->line(__('Total : :total :currency', ['total' => number_format((float) $this->order->total, 2), 'currency' => $currency]))

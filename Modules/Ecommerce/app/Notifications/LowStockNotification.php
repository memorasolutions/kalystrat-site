...
class LowStockNotification extends TemplatedNotification
{
    ...
    protected function getFallbackMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ->subject(__('Alerte stock bas — :sku', ['sku' => $this->variant->sku]))
            ->line(__('Le variant :sku du produit :product a atteint :stock unités (seuil : :threshold).', ['sku' => $this->variant->sku, 'product' => $this->variant->product->name, 'stock' => $this->variant->stock, 'threshold' => $this->variant->low_stock_threshold]))
            ->action(__('Voir le produit'), route('admin.ecommerce.products.edit', $this->variant->product_id));
    }

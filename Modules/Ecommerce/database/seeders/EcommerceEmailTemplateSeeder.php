    ...
    public static function defaults(): array
    {
        return [
            ...
            [
                ...
                'slug' => 'ecommerce_order_delivered',
                'subject' => 'Votre commande #{{order.number}} a été livrée',
                'module' => 'ecommerce',
                'variables' => ['user.name', 'user.email', 'app.name', 'app.url', 'order.number', 'order.url'],
                'body_html' => '<p style="font-size:16px;color:#333;margin:0 0 16px;">Bonjour {{user.name}},</p><p style="font-size:16px;color:#333;margin:0 0 16px;">Votre commande <strong>#{{order.number}}</strong> a été livrée avec succès!</p><p style="font-size:16px;color:#333;margin:0 0 24px;">Nous espérons que vous êtes satisfait(e) de votre achat.</p><p style="text-align:center;margin:24px 0;"><a href="{{order.url}}" style="background:#28a745;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;display:inline-block;font-weight:bold;">Voir ma commande</a></p><p style="font-size:14px;color:#666;margin:24px 0 0;">Merci pour votre confiance!</p>',
            ],
            [
                'name' => 'Alerte stock bas',
                'slug' => 'ecommerce_low_stock',
                'subject' => 'Alerte stock bas — {{product.name}} ({{product.sku}})',
                'module' => 'ecommerce',
                'variables' => ['product.name', 'product.sku', 'product.stock', 'product.threshold', 'product.id', 'app.name', 'app.url'],
                'body_html' => '<p style="font-size:16px;color:#333;margin:0 0 16px;">Alerte inventaire,</p><p style="font-size:16px;color:#333;margin:0 0 16px;">Le produit <strong>{{product.name}}</strong> (SKU: {{product.sku}}) est en stock bas.</p><table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;border-collapse:collapse;"><tr><td style="padding:10px 0;border-bottom:1px solid #eee;color:#666;">Stock actuel</td><td style="padding:10px 0;border-bottom:1px solid #eee;font-weight:bold;text-align:right;color:#dc3545;">{{product.stock}} unités</td></tr><tr><td style="padding:10px 0;color:#666;">Seuil d\'alerte</td><td style="padding:10px 0;font-weight:bold;text-align:right;">{{product.threshold}} unités</td></tr></table><p style="text-align:center;margin:24px 0;"><a href="{{app.url}}/admin/ecommerce/products/{{product.id}}/edit" style="background:#dc3545;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;display:inline-block;font-weight:bold;">Gérer le produit</a></p>',
            ],
            [

...
.PHONY: install dev test test-full test-coverage lint lint-check analyse rector rector-fix seed migrate fresh cache cache-clear horizon telescope docker-up docker-down docker-build deploy ide-helper check check-quick logs e2e setup-hooks setup warmup quality
...
test-full:
	php artisan view:cache
	php -d memory_limit=1G artisan test --parallel --exclude-group=sequential
	php artisan test --group=sequential
...
quality: lint-check analyse
	@echo "Code quality OK (Pint + PHPStan)"
